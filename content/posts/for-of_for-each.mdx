...
The `forEach()` method just like the `for...of` loop gives us the each element in the array as well as the opportunity to
pass a callback function to it which is called once for each element in the array.
...
//result: 6
```
Another interesting thing is that just like the `for...in` loop, the `forEach()` loop's callback function gives us an option to get the index (which in the case of arrays is the key) of the element like so:
...
//result: true
```
From the code above, the function `isEven` returns `true` because the first even number it encounters is `2` and it returns `true` immediately. This is the expected behavior. In a case where there are no even numbers in the array, the function returns `false` as expected. This is because the `return false` statement is outside the loop and only gets executed if the loop completes without returning `true`. We cannot say the same for the `forEach()` loop. Let us see why:
...
From the code above, the function `isEven` returns `false` even though there is an even number in the array. This is because the `return true` statement inside the `forEach` loop does not return from the `isEven` function but from the `forEach` loop. The `return true` statement only returns from the callback function passed to the `forEach` loop. The reason is on every iteration of the loop, the return values gets discarded and when the loop in done executing, control is transfered to the next statement outside of the loop, hence the `return false` statement eventually gets executed.
